...
gen upper = .
gen negative = .
gen gid = _n+1 in 1/21

forvalues i = 1/21 {
	quietly reg primary treatment i.year i.id if time<=`i', cluster(country)
	mat V = r(table)
	local j = `i'
	replace beta = V[1,1] in `j'
	replace lower = V[5,1] in `j'
	replace upper = V[6,1] in `j'
	quietly reg treatment i.year i.id if time<=`i'
...
twoway ///	
	...
	legend(off) xlabel(2(5)22, noticks) yline(0, lcolor(gs10) lwidth(thin)) ///
	ylabel(-20(20)80, noticks) xtitle(" " "Number of Post-Treatment Years Included") ///
...
graph export fig/effect-by-time-primary.pdf, replace
restore
...
forvalues i = 1/21 {
	quietly reg secondary treatment i.year i.id if time<=`i', cluster(country)
	mat V = r(table)
	local j = `i'
	replace beta = V[1,1] in `j'
	replace lower = V[5,1] in `j'
...
twoway ///	
	...
	(scatter beta gid, mcolor(sea*1.2) msymbol(s) msize(large)), ///
	legend(off) xlabel(2(5)22, noticks) yline(0, lcolor(gs10) lwidth(thin)) ///
	ylabel(-15(5)15, noticks) xtitle(" " "Number of Post-Treatment Years Included") ///
	ytitle("Estimated Treatment Effect" " ") ///
	ylabel(0 " " 5 " ", noticks labcolor(turquoise) axis(2) angle(0) labsize(small)) ///
	ytitle(" ", axis(2)) ///
	text(-11 2015.625 "Proportion of treatment observations receiving negative weight", color(turquoise) size(small) place(w))
graph export fig/effect-by-time-secondary.pdf, replace
