...
gen start = (`periods'/4) + ceil(`periods'*runiform())
...
** treatment effects
gen delta=2 // homogeneous
*gen delta = 3+ceil(5*runiform())

gen y = x + shock + delta*treatment + 0.005*rnormal()
...
graph export fig/example2-hist.pdf, replace
...
tw ///
	...
	legend(off)
graph export fig/example2-scatter-resid.pdf, replace	
